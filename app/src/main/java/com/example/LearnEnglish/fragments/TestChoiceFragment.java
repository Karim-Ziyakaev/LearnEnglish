...
public class TestChoiceFragment extends Fragment {
    ...
    private TestChoiceFragment.OnFragmentAnswerListener mListener;

    public void setOnFragmentAnswerListener(TestChoiceFragment.OnFragmentAnswerListener listener) {
        mListener = listener;
    }
    ...
    public void onAttach(Context context) {
        ...
        if (context instanceof TestChoiceFragment.OnFragmentAnswerListener) {
            mListener = (TestChoiceFragment.OnFragmentAnswerListener) context;
        } else {
    ...
    public static TestChoiceFragment getInstance(Word word, ArrayList<Word> words) {
    ...
    public static TestChoiceFragment newInstance() {
        TestChoiceFragment fragment = new TestChoiceFragment();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        return inflater.inflate(R.layout.fragment_test_choice, container, false);
    }
    ...
    private void init(){
        ...
        List<String> usedTranslation = new ArrayList<>();
        usedIndexes.add(mWords.indexOf(mWord));
        Button[] buttons = {button1, button2, button3, button4};

        for (int i = 0; i < NUM_BUTTONS; i++) {
            if (i == correctIndex) {
                buttons[i].setText(mWord.getTranslate());
                ...
                buttons[i].setText(mWords.get(randomIndex).getTranslate());
            }
            else{
                DatabaseAdapter db_adapter = new DatabaseAdapter(requireContext());
                db_adapter.open();
                Word temp = db_adapter.getRandomWord();
                while (usedTranslation.contains(temp.getTranslate())) {
                    temp = db_adapter.getRandomWord();
                }
                usedTranslation.add(temp.getTranslate());
                db_adapter.close();
                buttons[i].setText(temp.getTranslate());
            }
        }
